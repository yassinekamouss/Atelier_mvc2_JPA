...
<main class="mx-auto max-w-6xl px-4 py-8">
    <c:if test="${empty commande}">
        <div class="bg-white rounded shadow p-6 text-center text-gray-600">Commande introuvable.</div>
    </c:if>
    <c:if test="${not empty commande}">
        <div class="mb-6">
            <h1 class="text-2xl font-bold">Commande #${commande.id}</h1>
            <div class="text-sm text-gray-600 mt-1">Passée le <fmt:formatDate value="${commande.dateCommande}" type="both" dateStyle="medium" timeStyle="short"/> — Statut: <span class="font-medium text-gray-800">${commande.statut}</span></div>
        </div>
        ...
        <div class="bg-white rounded shadow overflow-hidden">
            ...
            <div class="p-4 border-t bg-gray-50 text-right text-lg">
                Total: <span class="font-bold text-blue-700"><fmt:formatNumber value="${commande.montantTotal}" type="currency" currencySymbol="€"/></span>
            </div>
        </div>

        <div class="mt-6">
            <a href="${pageContext.request.contextPath}/commande?action=history" class="text-blue-600 hover:underline">Retour à l'historique</a>
        </div>
    </c:if>
</main>
</body>
